/**
 * PHARMA Store Brand Configuration Package
...
const brands: Record<BrandKey, BrandConfig> = {
  ...
  // Future pharma/medical brands will be added here
};
...
export function getBrandByKey(key: string | BrandKey): BrandConfig | null {
  ...
  if (!Object.values(BrandKey).includes(key as BrandKey)) {
    console.warn(`[PHARMA Store] Invalid brand key: ${key}`);
    return null;
  }
...
/**
 * Default/fallback brand for PHARMA store
 */
export const DEFAULT_BRAND = BrandKey.PEPTIDES;
