/**
 * SHOFAR Store Runtime Resolution
...
import type { BrandConfig } from '@shofar/shofar-brand-config';
import {
  BrandKey,
  getBrandByKey,
  getBrandByHost,
...
} from '@shofar/shofar-brand-config';
import { headers, cookies } from 'next/headers';
import * as jose from 'jose';

// JWT secret for dev cookie override (only used in dev)
const getJwtSecret = (): Uint8Array => {
  const secret = process.env.JWT_SECRET || 'shofar-dev-secret-change-in-production';
 ...
 * 4. Default brand fallback (TOOLY)
 */
export async function resolveBrand(): Promise<BrandConfig> {
  ...
  if (process.env.BRAND_KEY) {
    ...
    if (brand) {
      console.log(`[SHOFAR Store] Resolved via BRAND_KEY env: ${brand.key}`);
      return brand;
    }
    console.warn(`[SHOFAR Store] Invalid BRAND_KEY env: ${process.env.BRAND_KEY}`);
  }
  ...
  if (host) {
    ...
    if (brand) {
      console.log(`[SHOFAR Store] Resolved via host '${host}': ${brand.key}`);
      return brand;
    }
  ...
  if (process.env.NODE_ENV === 'development' &&
    ...
    try {
      ...
      const cookieValue = cookieStore.get('shofar-brand-override')?.value;
      if (cookieValue) {
        ...
        if (payload.brand && isValidBrandKey(payload.brand as string)) {
          ...
          if (brand) {
            console.log(`[SHOFAR Store] DEV OVERRIDE via cookie: ${brand.key}`);
            return brand;
          }
        }
      }
    } catch (error) {
      console.error('[SHOFAR Store] Failed to verify dev override cookie:', error);
      // Continue to fallback
    }
  }

  // Fallback to default brand (TOOLY)
  const defaultBrand = getDefaultBrand();
  console.log(`[SHOFAR Store] Using fallback brand: ${defaultBrand.key}`);
  return defaultBrand;
}
